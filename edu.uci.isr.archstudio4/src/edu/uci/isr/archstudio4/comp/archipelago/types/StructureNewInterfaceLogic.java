...
//				super.run();
		...
		Action varientAction = new Action("Add Varient",AS.resources.getImageDescriptor(ArchstudioResources.ICON_TYPES)){
			@Override
			public void run() {
			...
			displayVarientTable(shell,eltRef,varientGroupRef);
			
			}
		};
		
		Action newProperty = new Action("New Propery",AS.resources.getImageDescriptor(ArchstudioResources.ICON_TYPES)){
			...
			public void run(){
				...
				Object list = AS.xarch.get(eltRef,"propertyGroup");
				ObjRef propertyGroupRef;
				if(list == null ){
					//Create New PropertyGroup
					propertyGroupRef = AS.xarch.create(typesContextRef, "propertyGroup");
					String changeID = UIDGenerator.generateUID("propertyGroup");				
					AS.xarch.set(propertyGroupRef, "id", changeID);
					
					AS.xarch.set(eltRef, "propertyGroup", propertyGroupRef);
					System.out.println("success");
				}else{
					 propertyGroupRef = (ObjRef)list;
				}
				...
				AS.xarch.add(propertyGroupRef,"property",property);
				...
				if(model.getSelectedFeature() == null){
					...
					try {
						...
						if(AS.xarch.isInstanceOf(featureRef, "features#Varient")){
							...
							guardRef = AS.booleanNotation.parseBooleanGuard(featureSymbol + " == \""+featureValue+"\"", xArchRef);
						}else if(AS.xarch.isInstanceOf(featureRef, "features#OptionalFeature")){
							
							guardRef = AS.booleanNotation.parseBooleanGuard(featureValue + " == \"true\"", xArchRef);
						}
						if(guardRef!=null)
		...
		return new IAction[]{/*newInterfaceAction*/newOptionalInterfaceAction,/*newProperty,*/propertyAction,varientAction};
	}

	
	protected void displayVarientTable(Shell parentShell, ObjRef eltRef,
			final ObjRef varientGroupRef) {
		...
		GridLayout cButtonsLayout = new GridLayout(6, false);
		...
		Label  nameLabel = new Label(cButtons, SWT.NULL);
		nameLabel.setText("Name:");
		final Text  nameText = new Text(cButtons, SWT.NULL);
		
		Label  descLabel = new Label(cButtons, SWT.NULL);
		descLabel.setText("Descripton:");
		final Text  descText = new Text(cButtons, SWT.NULL);
		...
		final ArrayList<ObjRef> varientList = new ArrayList<ObjRef>();
		
		GridData data = new GridData(SWT.FILL, SWT.FILL, true, true);
		data.heightHint = 200;
		table.setLayoutData(data);
		String[] titles = {"S.No ", " VarientType ", " Description  "};
		...
		for (int i = 0; i < varients.length; i++) {
			System.out.println(AS.xarch.get(varients[i%varients.length], "name"));	
			varientList.add(varients[i]);
			TableItem item = new TableItem (table, SWT.NONE);
			item.setText (0, i+"");
			item.setText (1, AS.xarch.get(varients[i%varients.length], "name").toString());
			item.setText (2, XadlUtils.getDescription(AS.xarch, varients[i%varients.length]));
...
//				System.out.println("value ->"+resl[2]);
				...
				ObjRef property = AS.xarch.create(typesContextRef, "varientItem");
				
				AS.xarch.set(property, "name", nameText.getText());
				XadlUtils.setDescription(AS.xarch, property,descText.getText());

				
				AS.xarch.add(varientGroupRef,"varientItem",property);
				
				
				varientList.add(property);
				
				TableItem item = new TableItem (table, SWT.NONE);
				item.setText (0, varientList.size()+"");
				item.setText (1, nameText.getText());
				item.setText (2, descText.getText());
